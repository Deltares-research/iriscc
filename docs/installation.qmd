...
If you're using D4Science environment, specify the `pixi` binary path manually in the terminal:
...
Start a notebook directly using Pixi, for example (only when working locally):
...
To use vscode in combination with the pixi environment created above, you can start vscode as:

```bash
pixi run code .
```

In this way vscode will inherit the environment variables to find the kernel. You can now select the kernel named "Python (iriscc)" in the notbook.